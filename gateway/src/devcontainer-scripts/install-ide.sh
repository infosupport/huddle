#!/usr/bin/env bash
#
# install-ide.sh - runs INSIDE the devcontainer, as the non-root dev user.
...
#
# Ported near-verbatim from pocsshcontainers/install-ide.sh. Embedded (base64,
# via gateway/src/devcontainer-scripts.ts) into the config script docker.ts
# execs as root right after the huddle CA is trusted, then run as `vscode`
# (this script refuses to run as root itself, see below) with
# CACHE_ROOT=/.jbdevcontainer so everything lands under the existing
# jb_devcontainers_shared_volume mount (/.jbdevcontainer/JetBrains/RemoteDev/dist)
# instead of $HOME/.cache, keeping warm-restart caching working unchanged.

set -euo pipefail

# The root guard exists for a shared, multi-user devcontainer (the backend
# forks the developer's shell as `vscode`, not root). It does not apply to an
# sbx sandbox, which has no separate user at all and always runs as root —
# HUDDLE_SBX_ROOT=1 is sbx.ts's explicit opt-out for that single-user case.
if [ "$(id -u)" = 0 ] && [ "${HUDDLE_SBX_ROOT:-}" != 1 ]; then
